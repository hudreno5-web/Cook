...
const allRecipes = [
    ...
    {
    name: " Motichur Laddu",
    img: "https://img.freepik.com/premium-photo/khas-khas-besan-laddu-poppy-seeds-chickpea-flour-laddo-laddoo_1093310-3004.jpg?w=740",
    time: "40 min (+freeze)",
    filter: "sweet"
},
    ...
    {
    name: "Chese Burger",
    img: "https://img.freepik.com/free-photo/still-life-delicious-american-hamburger_23-2149637318.jpg?t=st=1771761972~exp=1771765572~hmac=8f7d9f54e1551e8aa030241056c977f70192117b199c50957376c0efd54118a8&w=740",
    time: "15-20 min (+freeze)",
    filter: "non-veg"
},
    
    {
    name: " Ramen ",
    img: "https://img.freepik.com/free-photo/top-view-asian-ramen-noodles-bowl-black-pepper-small-bowl-cut-lemon-chopsticks-dark-table_140725-145845.jpg?t=st=1771761724~exp=1771765324~hmac=43a209d92cc433b19929e35881d9e1dc3b514e21a480d2a5758f5cf1a6428cd0&w=740",
    time: "20-30 min (+freeze)",
    filter: "non-veg"
},
    
    {
    name: " Sushi",
    img: "https://img.freepik.com/premium-photo/delicious-philadelphia-sushi-roll-with-salmon-shrimp-cucumber-cream-cheese_73989-58499.jpg?w=740",
    time: "45-60 min (+freeze)",
    filter: " non-veg "
},
    
    {
    name: "Dim sums, Momo",
    img: "https://img.freepik.com/free-photo/traditional-asian-food-plate_23-2148771247.jpg?t=st=1771761504~exp=1771765104~hmac=93f2613695d2c547bb87fed6cb3ad41b3b4d5b81673f926d05fc2da57a3a02ab&w=740",
    time: "45-50 min (+freeze)",
    filter: "non-veg"
},
    
    
    {
    name: "speedy egg,Quick Scramble",
    img: "https://images.pexels.com/photos/7393351/pexels-photo-7393351.jpeg?_gl=1*1cece5n*_ga*OTExMjU0NzQxLjE3NzE3NTc3NjY.*_ga_8JE65Q40S6*czE3NzE3NTc3NjYkbzEkZzEkdDE3NzE3NTg0MDMkajQ2JGwwJGgw",
    time: "5-10 min (+freeze)",
    filter: "non-veg"
},
    
    {
    name: "Bodega Sandwich",
    img: "https://images.pexels.com/photos/461382/pexels-photo-461382.jpeg?_gl=1*qzpshy*_ga*OTExMjU0NzQxLjE3NzE3NTc3NjY.*_ga_8JE65Q40S6*czE3NzE3NTc3NjYkbzEkZzEkdDE3NzE3NTc4NDckajU5JGwwJGgw",
    time: "10-15 min (+freeze)",
    filter: "non-veg"
},
    ...
    { name: " odisha dahi bara aloo dum ", img: "https://content.jdmagicbox.com/v2/comp/bhubaneshwar/l2/0674px674.x674.180109200346.j5l2/catalogue/mitu-bhai-dahibara-aloodum-satya-nagar-bhubaneshwar-fast-food-LuXae98TE5.jpg", time: "120 min", filter: "veg" }
...
function renderRecipes() {
    ...
    const attachLongPress = (card, recipeName) => {
        let pressTimer;

        const startPress = (e) => {
            // Agar user move kar raha hai toh timer start na ho
            pressTimer = setTimeout(() => {
                showRecipeFact(recipeName); // Aapka AI fact function call hoga
                if ('vibrate' in navigator) navigator.vibrate(50); // Haptic feedback
            }, 600); // 600ms hold karne par open hoga
        };
        ...
        card.addEventListener('contextmenu', (e) => {
            e.preventDefault(); // Default menu ko rokne ke liye
            showRecipeFact(recipeName);
        });
    };
    // --- LONG PRESS LOGIC END ---

    // 1. Trending Grid Render
    trendingRecipes.forEach(recipe => {
        ...
        `;
        // Long press logic attach karna
        attachLongPress(card, recipe.name);
        container.appendChild(card);
    });
    
    // 2. All Recipes List Render
...
try {
    // We added 'await' here so it pauses slightly to fetch the video ID!
    const systemPrompt = await getSmartSystemPrompt(userMessage, currentLanguage); 
...
window.toggleSidebar = () => {
    ...
    if (!sidebar.classList.contains('open')) {
        // Step 1: Render the history BEFORE opening to prevent main-thread freezing
        renderSidebarHistory();
        
        // Step 2: Wait for the next browser paint frame, then animate smoothly
...
// Prevent Right Click and F12 (Basic Deterrence)
...
if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => {
        // { updateViaCache: 'none' } ensures browser hamesha naya sw.js server se check kare (cache se nahi)
        navigator.serviceWorker.register('sw.js', { updateViaCache: 'none' })
            .then(registration => {
                console.log('Chef AI: Service Worker registered successfully with scope:', registration.scope);
                
                // Naya update check karne ka logic
                registration.addEventListener('updatefound', () => {
                    const newWorker = registration.installing;
                    newWorker.addEventListener('statechange', () => {
                        // Jab naya service worker activate ho jaye
                        if (newWorker.state === 'activated') {
                            console.log('New update found and activated!');
                            showToast("App is updating to a new version... 🚀");
                        }
                    });
                });
            })
    ...
    // YEH SABSE ZAROORI HAI: Jab naya Service Worker control le, toh app ko automatically reload kar do
...
// App load hone par ek dummy history state push karo
window.addEventListener('load', () => {
...
window.addEventListener('popstate', (event) => {
    if (!backPressedOnce) {
        // Pehli baar back dabaya
        backPressedOnce = true;
        showMaterialToast("Press back again to exit");
        
        // Wapas ek state push kardo taaki app band na ho
        window.history.pushState({ noBackExitsApp: true }, '');
        
        // 2 second baad reset kar do
        setTimeout(() => {
            backPressedOnce = false;
        }, 2000);
    } else {
        // Dusri baar dabaya 2 second ke andar, let it exit
        window.history.back(); 
    }
});

// Toast dikhane ka function
function showMaterialToast(message) {
    const toast = document.getElementById('material-toast');
    toast.textContent = message;
...
// ==========================================
// ADVANCED ANTI-HACKER TRAP
// ==========================================

// 1. Console mein 👎 print karna (Agar koi F12 khol le toh sabse pehle ye dikhega)
console.log("%c👎 👎 👎\nKahan ghus rahe ho bhai? Code protected hai!", "color: red; font-size: 40px; font-weight: bold; text-shadow: 2px 2px 0 #000;");
console.log("%cYe harkat theek nahi hai.", "font-size: 20px;");

// 2. Debugger Trap & Screen Wipe (Agar DevTools open hua toh screen par 👎 dikhega)
setInterval(function() {
    const before = new Date().getTime();
    
    // Yeh line devtools open hone par page ko freeze kar deti hai
    debugger; 
    
    const after = new Date().getTime();
    
    // Agar time difference 100ms se zyada hai, matlab DevTools open hai!
    if (after - before > 100) {
        document.body.innerHTML = `
            <div style="height: 100vh; width: 100vw; background: black; display: flex; flex-direction: column; align-items: center; justify-content: center; color: red; font-family: sans-serif;">
                <h1 style="font-size: 100px; margin: 0;">👎 👎 👎</h1>
                <h2>Nice try, Hacker!</h2>
                <p>But this app is protected.</p>
            </div>
        `;
    }
...
// ============================================================
// AUTOMATIC RECIPE FACT GENERATOR (AI POWERED) - FIXED
// ============================================================

// Memory (Cache) so we don't call the AI twice for the same dish
const recipeFactsCache = {};

async function showRecipeFact(recipeName) {
    const modal = document.getElementById('recipe-fact-modal');
    const titleEl = document.getElementById('fact-modal-title');
    const countryEl = document.getElementById('fact-modal-country');
    const factEl = document.getElementById('fact-modal-text');
    const lightbulb = document.getElementById('fact-lightbulb-icon');

    // 1. Show Loading State First
    titleEl.textContent = recipeName;
    countryEl.textContent = "Loading Origin...";
    factEl.textContent = "AI is cooking up an interesting fact...";
    if (lightbulb) lightbulb.style.animation = "pulse 1s infinite alternate"; 
    modal.style.display = 'flex';

    // Map language code to full name for the AI prompt
    const langMap = { en: "English", hi: "Hindi", hinglish: "Hinglish", or: "Odia", zh: "Chinese" };
    const langName = typeof currentLanguage !== 'undefined' ? (langMap[currentLanguage] || "English") : "English";
    
    const cacheKey = `${recipeName}_${langName}`;

    // 2. Check if already loaded in memory (Saves API calls & loads instantly)
    if (recipeFactsCache[cacheKey]) {
        countryEl.textContent = recipeFactsCache[cacheKey].country;
        factEl.textContent = recipeFactsCache[cacheKey].fact;
        if (lightbulb) lightbulb.style.animation = "none";
        return;
    }

    // 3. Real API Call to OpenRouter
    try {
        const prompt = `Give me the country of origin and 1 very interesting fun fact about the dish "${recipeName}". The response MUST be in ${langName} language. Reply EXACTLY in this format: Country Name (with 1 flag emoji) | Fun Fact. Do not include any other markdown or text.`;
        ...
        const response = await fetch("https://openrouter.ai/api/v1/chat/completions", {
            ...
            body: JSON.stringify({
                model: AI_MODEL,
                messages: [
                    { role: 'system', content: 'You are a culinary expert AI. You strictly follow formatting rules.' },
        ...
        // 4. Extract Country and Fact
        let country = "Unknown Origin 🌍";
        let fact = botReply;
        ...
        recipeFactsCache[cacheKey] = { country, fact };
        ...
        countryEl.textContent = "Oops! 🔌";
        factEl.textContent = "Could not fetch the fact right now. Please try again.";
    } finally {
...
// Attach close function to the global window so HTML onclick can find it
window.closeRecipeFactModal = function() {
    const modal = document.getElementById('recipe-fact-modal');
    if (modal) {
